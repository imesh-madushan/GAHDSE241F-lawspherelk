...
test('bulk create complaints with different users', async () => {
  ...
  // Fill login form (using existing user from your data)
  await page.fill('input[name="username"]', 'user1');
  await page.fill('input[name="password"]', 'password123'); // The password that matches the hash
  await page.click('button[type="submit"]');
  ...
  for (let i = 0; i < complaints.length; i++) {
    ...
    console.log(`\n=== Creating Complaint ${i + 1}/${complaints.length} ===`);
    console.log(`Type: ${complaintType}`);
    console.log(`Description: ${c.description}`);
    ...
    await page.waitForTimeout(100);
    await page.fill('textarea[name="evidence_details"]', c.evidence_details);
    await page.waitForTimeout(200);// Wait for you to click "Submit Complaint" manually
    console.log(`Waiting for you to click "Submit Complaint" for complaint ${i + 1}/${complaints.length}: ${c.description.substring(0, 50)}...`);
    await page.waitForSelector('text=Complaint Created Successfully', { timeout: 120000 }); // 2 minutes timeout
