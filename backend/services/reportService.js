...
const generateCaseReportHTML = async (caseData) => {
  ...
  const stats = {
    ...
    completedInvestigations:
      caseData.investigations?.filter((inv) => inv.status === "completed")
        .length || 0,
    convictedOffences:
      caseData.offences?.filter((off) => off.status === "Convicted").length ||
      0,
    totalRiskScore:
      caseData.offences?.reduce((sum, off) => sum + (off.risk_score || 0), 0) ||
      0,
  };
  ...
  const html = `
    ...
    <body>
      ...
      <div class="header">
        ...
        <p style="margin-top: 10px; color: #6b7280;">Generated on ${formatDate(
          new Date()
        )}</p>
      </div>
            ...
            <div class="info-item">
              ...
              <span class="info-value">
                <span class="status-badge" style="background-color: ${getStatusColor(
                  caseData.status
                )}; color: white;">${caseData.status}</span>
              </span>
            </div>
          </div>
          <div>
            ...
            <div class="info-item">
              ...
              <span class="info-value">${caseData.leader_name || "N/A"} (${
    caseData.leader_role || "N/A"
  })</span>
            </div>
            <div class="info-item">
      ...
      <div class="section">
        ...
        <div style="background: #f8fafc; padding: 20px; border-radius: 8px; border-left: 4px solid #1f2937;">
          <p style="margin-bottom: 15px; font-size: 14px; line-height: 1.8;">
            This case report provides a comprehensive overview of Case ${
              caseData.case_id
            } - "${caseData.topic || "Untitled Case"}". 
            The case was initiated on ${formatDateOnly(
              caseData.started_dt
            )} and is currently in <strong>${caseData.status}</strong> status.
          </p>
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin: 15px 0;">
            <div>
              ...
              <ul style="color: #6b7280; font-size: 13px; line-height: 1.6;">
                ...
                <li>${stats.completedInvestigations} of ${
    stats.totalInvestigations
  } investigations completed</li>
                <li>${stats.convictedOffences} convictions out of ${
    stats.totalOffences
  } offences</li>
            ...
            <div>
              ...
              <div style="color: #6b7280; font-size: 13px; line-height: 1.6;">
                <div style="margin-bottom: 8px;">
                  Total Risk Score: <strong style="color: ${
                    getRiskLevel(stats.totalRiskScore).color
                  };">${stats.totalRiskScore} (${
    getRiskLevel(stats.totalRiskScore).level
  })</strong>
                </div>
                <div>Case Type: <strong>${
                  caseData.case_type || "N/A"
                }</strong></div>
                ${
                  stats.totalOffences > 0
                    ? `<div style="margin-top: 5px; font-size: 12px; color: #9ca3af;">Based on ${stats.totalOffences} recorded offences</div>`
                    : ""
                }
              </div>
            </div>
          ...
          ${
            ...
            <div style="margin-top: 15px; padding: 10px; background: #d1fae5; border-radius: 4px; border: 1px solid #10b981;">
              ...
              <span style="color: #047857; font-size: 13px;">This case has been successfully closed on ${formatDateOnly(
                caseData.end_dt
              )}.</span>
            </div>
          `
            ...
            <div class="info-grid">
              ...
              <div>
                <div class="info-item">
                  ...
                  <span class="info-value">${
                    caseData.complaint.officer_name || "N/A"
                  } (${caseData.complaint.officer_role || "N/A"})</span>
                </div>
                <div class="info-item">
                  <span class="info-label">Status:</span>
                  <span class="info-value">
                    <span class="status-badge status-${
                      caseData.complaint.complaint_status
                    }">${caseData.complaint.complaint_status}</span>
                  </span>
                </div>
              </div>
            </div>
            ...
            <tbody>
              ${caseData.investigations
                ...
                <tr>
                  ...
                  <td>
                    <span class="status-badge status-${investigation.status}">${
              ...
              ${caseData.offences
                .map((offence) => {
                  ...
                  <tr>
                    ...
                    <td>
                      <span class="status-badge status-${offence.status?.toLowerCase()}">${
                    offence.status || "N/A"
                  }</span>
                    </td>
                    <td>
            ...
            <tbody>
              ${caseData.reports
                ...
                <tr>
                  ...
                  <td>${report.created_by || "N/A"} (${
                    report.officer_role || "N/A"
                  })</td>
                  ...
                  <tr>
                    <td colspan="5">
                      <div class="evidence-details">
                        ...
                        ${
                          report.remarks
                            ? `<br><strong>Remarks:</strong> ${report.remarks}`
                            : ""
                        }
                      </div>
                    </td>
      ...
      <div class="section" style="margin-top: 40px; text-align: center; color: #6b7280; font-size: 12px;">
        ...
        <p>Generated on ${formatDate(new Date())} | Case ID: ${
    caseData.case_id
  }</p>
