...
exports.logAuditTrail = async (auditConfig) => {
    ...
    if (!batchId || !changes || !changedBy) {
        throw new Error("Missing required audit parameters: batchId, changes, changedBy");
    }
                ...
                for (const change of changes) {
                    ...
                    await conn.query(
                        `INSERT INTO audit_log (audit_id, batch_id, table_name, record_id, field_name, value, action_type, changed_by, changed_at)
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
                        [auditId, batchId, change.tableName, change.recordId, change.fieldName, change.value, change.actionType, changedBy]
                    );
                }
            }
            // Handle single change object with tableName, recordId, fieldName, value
            else if (typeof changes === 'object' && changes !== null && changes.tableName && changes.recordId) {
                const auditId = await generateUniqueId("audit_log");
                
                await conn.query(
                    `INSERT INTO audit_log (audit_id, batch_id, table_name, record_id, field_name, value, action_type, changed_by, changed_at)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
                    [auditId, batchId, changes.tableName, changes.recordId, changes.fieldName, changes.value, changes.actionType, changedBy]
                );
            }
            // Fallback for other formats (backward compatibility)
            else {
                throw new Error("Invalid changes format. Expected array or object with tableName, recordId, fieldName, value");
            }
