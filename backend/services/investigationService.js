...
exports.createInvestigation = async (investigationData, createdBy) => {
    ...
    try {
        ...
        await connection.query(`
            INSERT INTO investigation (investigation_id, topic, start_dt, location, status, case_id)
            VALUES (?, ?, NOW(), ?, 'inprogress', ?)
        `, [investigationId, topic, location, case_id]);

        // Add officers to investigation and collect audit logs
        let officerAuditChanges = [];
        if (officer_ids.length > 0) {
            for (const officerId of officer_ids) {
                await connection.query(`
                    INSERT INTO investigation_officer (investigation_id, officer_id)
                    VALUES (?, ?)
                `, [investigationId, officerId]);
                officerAuditChanges.push({
        ...
        return { investigation_id: investigationId, ...investigationData };
    } catch (error) {
...
exports.updateInvestigation = async (investigationId, updateFields, updatedBy) => {
    const connection = await db.getConnection();
    try {
        await connection.beginTransaction();
        ...
        for (const key of Object.keys(updateFields)) {
            if (updateFields[key] !== undefined && updateFields[key] !== currentInvestigation[key]) {
...
exports.addOfficerToInvestigation = async (investigationId, officerId, addedBy) => {
    const connection = await db.getConnection();
    try {
        await connection.beginTransaction();
...
exports.removeOfficerFromInvestigation = async (investigationId, officerId, removedBy) => {
    const connection = await db.getConnection();
    try {
        await connection.beginTransaction();
